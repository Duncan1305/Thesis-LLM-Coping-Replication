...
# (Voeg Optional toe aan de import bovenaan de class,
# of aan de algemene 'typing' import bovenaan het script)

class Response_Baseline(BaseModel):
    always_relevant_score: int
    certain_conditions_score: int
    never_relevant_score: int

class Response_With_Rationale(BaseModel):
    rationale: str
    always_relevant_score: int
    certain_conditions_score: int
    never_relevant_score: int
...
def load_prompts_from_file(file_path: str) -> Dict[str, str]:
    """Leest prompts uit een tekstbestand en geeft een dictionary terug."""
    prompts = {}
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()

    # Splitst het bestand op basis van de ### ID ### markers
    prompt_sections = re.split(r'###\s*(.*?)\s*###', content)

    # Negeer de eerste lege string en verwerk de rest in paren (ID, Tekst)
    for i in range(1, len(prompt_sections), 2):
        prompt_id = prompt_sections[i].strip()
        prompt_text = prompt_sections[i + 1].strip()
...
def generate_LLM_ratings(
    combinations_df: pd.DataFrame,
    system_prompt: str,
    prompt_id: str,  # <--- NIEUW: We geven de ID mee
    max_workers: int = 5,
    LLM: str = "gpt-4o-mini",
    verbose_mode: bool = False,
    test_mode: bool = True,
):
    logging.info(f"Generating LLM scores for {prompt_id}...")

    # KIES HIER HET MODEL: Alleen Baseline krijgt geen rationale
    current_model = Response_Baseline if prompt_id == "PROMPT_BASELINE" else Response_With_Rationale
    ...
    def wrapped_call(idx: int, user_query: str):
        result = call_GPT(
            ...
            pydantic_model=current_model, # <--- Gebruik het gekozen model
            model=LLM,
        )
...
def compute_correlation_and_confusion(
        ...
        corr_rows.append(
            {
                ...
                "certain_conditions_USER": float(user_vals[1]),
                "never_USER": float(user_vals[2]),
                "always_LLM": resp.always_relevant_score,
                "certain_conditions_LLM": resp.certain_conditions_score,
                "never_LLM": resp.never_relevant_score,
            }
        )

        # Ground Truth Label (van User)
        user_scores = {'relevant': user_vals[0] + user_vals[1], 'irrelevant': user_vals[2]}
        true_labels.append(max(user_scores, key=user_scores.get))

        # Predicted Label (van LLM)
        llm_scores_dict = {'relevant': resp.always_relevant_score + resp.certain_conditions_score,
                           'irrelevant': resp.never_relevant_score}
        pred_labels.append(max(llm_scores_dict, key=llm_scores_dict.get))
    ...
    # ------------------------------------------------------------------ #
    numeric_cols = ["always_USER", "certain_conditions_USER", "never_USER", "always_LLM", "certain_conditions_LLM", "never_LLM"]
...
def grouped_metrics(
    ...
    def _group_loop(level: str, grp_dir: str, col_name: str) -> None:
        for grp in corr_df[col_name].dropna().unique():
            ...
            # correlation
            sub_corr = corr_df.loc[idxs,
            ["always_USER", "certain_conditions_USER", "never_USER", "always_LLM", "certain_conditions_LLM", "never_LLM"]].corr("pearson")
            fig, ax = plt.subplots(figsize=(8, 7))
            ...
            pred_labels = []
            for i in idxs:
                # Haal de gebruikersratings op voor de huidige index
                user_vals = combinations_df.loc[i, "Aggregated User Rating"]
                # Bepaal het 'ware' label: is de som van 'always' en 'conditions' groter dan 'never'?
                user_scores = {'relevant': user_vals[0] + user_vals[1], 'irrelevant': user_vals[2]}
                true_labels.append(max(user_scores, key=user_scores.get))

                # Haal het LLM-antwoord op voor de huidige index
                resp = llm_scores[i]
                # Bepaal het 'voorspelde' label op dezelfde manier
                llm_scores_dict = {'relevant': resp.always_relevant_score + resp.certain_conditions_score,
                                   'irrelevant': resp.never_relevant_score}
                pred_labels.append(max(llm_scores_dict, key=llm_scores_dict.get))
...
def save_llm_results_excel(
        combinations_df: pd.DataFrame,
        llm_scores: List, # List van eender welk model
        output_file: str,
) -> None:
    records = []
    for i, resp in enumerate(llm_scores):
        records.append(
            {
                ...
                # Gebruik getattr om te kijken of rationale bestaat, anders "N/A"
                "Rationale": getattr(resp, 'rationale', "N/A (Baseline)"),
                "Score_Always": resp.always_relevant_score,
                "Score_Conditions": resp.certain_conditions_score,
                "Score_Never": resp.never_relevant_score,
            }
        )
...
if __name__ == "__main__":
    # --- CONFIGURATIE VAN DE RUN ---
    # Kies hier welke prompt je wilt testen uit je prompts.txt bestand
    PROMPT_ID_TO_TEST = "PROMPT_BASELINE"  # <-- VERANDER DIT VOOR ELK EXPERIMENT
    TEST_MODE_ENABLED = False  # <-- Zet op False voor de volledige, definitieve run
    ...
    # --- Laad prompts ---
    all_prompts = load_prompts_from_file("prompts.txt")
    selected_prompt = all_prompts[PROMPT_ID_TO_TEST]
    logging.info(f"Geselecteerde prompt voor deze run: {PROMPT_ID_TO_TEST}")

    # ----- 1. Create combinations -------------------------------------------------
    file_path = "C:/Users/dunca/OneDrive/Unif/Masterproef 2024-2026/MappingComparison_UsersLLM-main/OSF_data/relevance/relevance_by_combination.csv"
    combinations_df = create_combinations(file_path, verbose_mode=False)

    # ----- 2. LLM evaluation ------------------------------------------------------
    llm_scores = generate_LLM_ratings(
        combinations_df,
        system_prompt=selected_prompt,  # <-- Geeft de geselecteerde prompt mee
        prompt_id=PROMPT_ID_TO_TEST,
        max_workers=2,  # Veilige waarde om rate limits te vermijden
        LLM="gpt-4o-mini",
        verbose_mode=True,
    ...
    # Maakt een aparte output map voor elke prompt-test
    base_output_dir = f"C:/Users/dunca/OneDrive/Unif/Masterproef 2024-2026/MappingComparison_UsersLLM-main/results/{PROMPT_ID_TO_TEST}"
    full_dir = os.path.join(base_output_dir, "full")
    os.makedirs(full_dir, exist_ok=True)
    logging.info(f"Resultaten worden opgeslagen in: {base_output_dir}")
    ...
    grouped_metrics(corr_df, combinations_df, llm_scores, base_output_dir)

    # ----- 4. Save raw LLM results to Excel -------------------------------------
    excel_output_path = os.path.join(full_dir, "llm_raw_results.xlsx")
    save_llm_results_excel(combinations_df, llm_scores, excel_output_path)

    logging.info("Done – all correlation & confusion matrices and Excel file saved.")
